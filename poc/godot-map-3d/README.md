...
- CC0の原本ZIPは `assets/source_models/toilet_sink/` で管理する。初回は `bash prepare_models.sh` で `assets/models/toilet_sink/toilet_sink.blend` を展開し、Godotが1マス内へ自動縮尺する。Actionsでは自動実行し、読込失敗時のみ軽量な簡易形状へ戻す。
